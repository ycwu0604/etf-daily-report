...
.DESCRIPTION
    Runs the full pipeline:
      1. Ezmoney: download xlsx → parse into SQLite DB
      2. Capital: API fetch → write into SQLite DB
      3. Combined: read all from DB → generate combined 4-ETF report
    
    All ETF data now flows through the shared SQLite DB (Ezmoney/etf_data.db).
...
# ═══════════════════════════════════════════════════════════════════════
# Step 1: Ezmoney — download xlsx → parse into DB
# ═══════════════════════════════════════════════════════════════════════
Write-Log "--- Step 1: Ezmoney (download → DB) ---"
...
# ═══════════════════════════════════════════════════════════════════════
# Step 2: Capital — API fetch → write into DB
# ═══════════════════════════════════════════════════════════════════════
Write-Log "--- Step 2: Capital (API → DB) ---"
$capitalScript = Join-Path $capitalDir "run_daily.py"
...
# ═══════════════════════════════════════════════════════════════════════
# Step 3: Combined report (all from DB)
# ═══════════════════════════════════════════════════════════════════════
Write-Log "--- Step 3: Combined report (from DB) ---"
